...
**Themes:** 8 themes, picker order light `[maple, spruce, linen, milk]`, dark `[mahogany, walnut, blueprint, void]`; the dark-mode switch uses `TunerThemes.darkModePairs`. The four wood themes carry a `WoodFinish` (`theme.wood`, `theme.isWood`) — every wood-only treatment (grain via `WoodSurface`/`WoodGrainPainter` in `widgets/wood_surface.dart`, gold frame, neck rail + tuning pins, gold-leaf buttons) is gated on it, so the flat themes keep their flat look. Two things are deliberately shared by all themes. First, the harp neck rail with tuning pins above the strings: wood themes get a grained rail with gold pins, and flat themes get a plain `surfaceHi → surfaceRim` rail with silver pins. See `TunerThemeData.neck` / `HarpNeck`. Second, the active-string glow. Don't restyle flat themes beyond that when touching wood code.
